...
{ 
	...
	unsigned int texID = loadTexture("textures/haikyuu.jpg");
...
unsigned int SceneManager::loadTexture(string filename)
{
	...
	if (data)
	{
		...
		for (int i = 0 ; i < size ; i += nrChannels) {
			...
			average = data[i]*0.2125 + data[i + 1]*0.7154 + data[i + 2]*0.0721;
			binary  = average > 84 ? 255 : 0;
